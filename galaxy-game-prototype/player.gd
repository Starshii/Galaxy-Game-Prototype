...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion and isIntelescope:
		telescopeCam.rotation.x -= event.relative.y * SENSITIVITY
		telescopeCam.rotation.y -= event.relative.x * SENSITIVITY
...
func _physics_process(delta: float) -> void:
	...
	var direction := (telescopeCam.transform.basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
	if direction:
...
func OutTelescope() -> void: 
	playerCam.make_current()
	telescopeCam.rotation = Vector3.ZERO
	isIntelescope = false
